...
class AuthService {
    ...
    /**
     * Rôles reconnus comme manager (dashboard département).
     */
    protected function isManagerRole(string $role): bool {
        return in_array(strtolower(trim($role)), ['manager', 'gerant', 'gérant'], true);
    }
    ...
    public function login(string $email, string $password) {
        ...
        $departementId = null;
        $deptAsManager = $this->departementDao->findByManagerId((int)$user['id']);
        if ($deptAsManager && isset($deptAsManager['id'])) {
            $departementId = (int)$deptAsManager['id'];
        }

        $employee = $this->employeeDao->findByUserId($user['id']);
        if ($departementId === null && $employee && isset($employee['departement_id'])) {
            $departementId = (int)$employee['departement_id'];
        }

        $role = strtolower(trim((string)($user['role'] ?? '')));
        $isDepartmentManager = $deptAsManager !== null;

        if ($isDepartmentManager && !in_array($role, ['patron', 'admin'], true)) {
            $role = 'manager';
        }
    ...
    public function isManager(): bool {
        ...
        $user = $_SESSION['user'];
        if (!empty($user['isDepartmentManager'])) {
            return true;
        }
    ...
    /**
     * Réservé au manager (ou admin) avec un département associé.
     */
    public function requireManager(): void {
        ...
        if (!$this->isManager() && !$this->isAdmin()) {
            ...
            header('Content-Type: application/json');
            echo json_encode(['status' => 'error', 'message' => 'Accès réservé au manager']);
            exit;
        }
        if ($this->getUserDepartementId() === null) {
            http_response_code(403);
            header('Content-Type: application/json');
            echo json_encode(['status' => 'error', 'message' => 'Aucun département associé à ce compte']);
            exit;
        }
    }
