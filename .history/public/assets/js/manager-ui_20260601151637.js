...
(function () {
    ...
    /**PAGINATON */
    // Dans manager-ui.js, après le chargement du fragment employees-content.html
...
async function loadEmployees(page = 1) {
    ...
    try {
        const res = await ManagerAPI.employees.list({ page: page, limit: 10 });
        const employees = res.data || [];
        const pagination = res.pagination || {};

        if (!employees.length) {
            tbody.innerHTML = '<tr><td colspan="7" class="mgr-empty-hint">Aucun employé</div></td></tr>';
            if (employeePagination) employeePagination.update(1, 1);
            return;
        }

        // Remplir le tableau
        tbody.innerHTML = employees.map(e => `...`).join('');

        // Initialiser ou mettre à jour la pagination
        if (!employeePagination) {
            employeePagination = new PaginationManager({
                containerId: 'employees-pagination',
                loadCallback: (newPage) => loadEmployees(newPage)
            });
        }
        employeePagination.update(pagination.currentPage, pagination.totalPages);
    } catch (err) {
        console.error(err);
        tbody.innerHTML = `<table><td colspan="7" class="mgr-empty-hint tft-clr-red">Erreur</td></tr>`;
    }
}
