...
async function loadEmployees(page = 1) {
    ...
    try {
        ...
        if (!employees.length) {
            tbody.innerHTML = '<tr><td colspan="7" class="mgr-empty-hint">Aucun employé trouvé</div></td></tr>';
        ...
        employeePagination.update(pagination.currentPage || 1, pagination.totalPages || 1);
    } catch (err) {
        console.error(err);
        tbody.innerHTML = `<tr><td colspan="7" class="mgr-empty-hint tft-clr-red">Erreur : ${escapeHtml(err.message)}</div></td></tr>`;
    }
}
